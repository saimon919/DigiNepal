...
export default function AdminDashboard() {
    ...
    const handleMainImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files?.[0]) {
            ...
            if (url) {
                ...
                alert("Image upload failed! Make sure the server is running on port 3001.");
            }
            setUploading(false);
        }
    };
    ...
    const handleGalleryChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            for (const file of files) {
                ...
                if (url) {
                    setNewProduct(prev => ({ ...prev, additionalImages: [...prev.additionalImages, url] }));
                }
            }
            setUploading(false);
        }
    ...
    const handleDigitalFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files?.[0]) {
            ...
            if (url) {
                ...
                alert("Digital file upload failed! Check server connection.");
            }
            setUploading(false);
        }
    };
    ...
    const handleAddProduct = async (e: React.FormEvent) => {
        ...
        if (!newProduct.image || !newProduct.downloadUrl) {
            alert("Please wait for images and the digital file to finish uploading!");
            return;
        }
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className={`relative h-20 rounded-2xl border-2 border-dashed flex items-center justify-center transition-all cursor-pointer ${newProduct.downloadUrl ? 'bg-green-50 border-green-200 text-green-600' : 'bg-bg border-gray-200 text-text-dim hover:border-primary/30'}`}>
                                        <div className="flex items-center gap-3">
                                            {newProduct.downloadUrl ? <CheckCircle className="text-green-500" /> : <FileText size={20} />}
